...
const colorScript = (css: Stitches.CSS) => {
  ...
  return Object.values(theme.colors).reduce((prev, color) => {
    ...
    const replacedSubstring = substringToReplace[0].replace(COLOR_PATTERN, `$${color.token}`)

    const replacedString = cssString.replace(substringToReplace[0], replacedSubstring)

    return {...prev, [color.token]: JSON.parse(replacedString)}
  }, {}) as Record<keyof typeof theme.colors, Stitches.CSS>
}

export const generateColorArray = colorScript
